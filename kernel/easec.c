...
void error_at(EasecToken* token, const char* message) {
    if (had_error) return;
    printf("Parse error on line %d: %s. Got '%s'\n", token->line, message, token->text);
    had_error = 1;
}
    ...
    switch (expr->type) {
        ...
        case EXPR_BINOP: {
            ...
            switch (expr->as.bin.op) {
                ...
                case TOKEN_BANGEQ: {
                    write_chunk(compiler->chunk, OP_EQUAL, expr->line); 
                    write_chunk(compiler->chunk, OP_NOT, expr->line); 
                    break;
                }
...
void run_script(const char* source, Env* env) {
    ...
    if (run() == INTERPRET_RUNTIME_ERROR) printf("Execution aborted due to runtime error.\n");
